...
export const TimelineItem: React.FC<TimelineItemProps> = ({ date, title, subTitle, isCourse, link, tag }) => {
    return (
        <div className="flex flex-wrap gap-12 min-h justify-start relative text-base sm:text-md">
            {/* Date Timeline */}
            <div className="h-auto w-fit flex-none whitespace-pre" style={{ width: `${isCourse ? "0" : "auto"}` }}>
                <p className="text-secondary-foreground">{date}</p>
            </div>
            {/* Right side */}
            <div className="flex gap-x-2 w-fit" style={{ transform: `${isCourse ? "translateX(-45px)" : "none"}` }}>
                <div className="flex flex-col gap-y-0.5">
                    {/* Title */}
                    <div className="break-words w-fit whitespace-pre">
...
const Body: React.FC<BodyProps> = ({ subTitle, tag, link }) => {
    return (
        <div className="flex items-center">
            <p className="text-lg w-fit font-normal leading-6 mt-1">{subTitle}</p>
